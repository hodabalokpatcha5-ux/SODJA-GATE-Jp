...
const notifyAdmin = async (subject: string, message: string, type: 'order' | 'general' = 'general') => {
  const adminEmails = ["sodjagate@gmail.com", "sodjagatecommande@gmail.com"];
...
// Database setup - will be initialized inside startServer
  ...
  try {
    const orderId = await db.transaction(async () => {
      const orderResult = await db.run("INSERT INTO orders (user_id, total_amount, delivery_fee, user_lat, user_lng) VALUES (?, ?, ?, ?, ?)", 
        [req.user.id, total_amount, delivery_fee || 0, user_lat || null, user_lng || null]);
      ...
      await db.run("INSERT INTO payments (order_id, amount, method, status) VALUES (?, ?, ?, ?)", 
        [orderId, total_amount, payment_method, 'Confirmé']);
...
async function startServer() {
  ...
  // 1. Start listening IMMEDIATELY to satisfy the proxy health checks
  const server = app.listen(Number(PORT), "0.0.0.0", () => {
    console.log(`🚀 [Server] HTTP server is now listening on http://0.0.0.0:${PORT}`);
  });

  // 2. Initialize Database after listening
  try {
    ...
    if (process.env.DATABASE_URL) {
      ...
      const sqlite = new Database("sodjagate.db");
      db = {
        ...
        transaction: async (fn) => {
          // Note: better-sqlite3 transactions are sync, but we wrap them in async for consistency
          const transaction = sqlite.transaction((...args: any[]) => {
            // This is a bit tricky because fn is async. 
            // For SQLite we'll just execute it and hope for the best, 
            // or use manual BEGIN/COMMIT if needed.
            return fn();
          });
          return transaction();
        }
  ...
  // 4. Vite middleware for development (Last, because it's heavy)
